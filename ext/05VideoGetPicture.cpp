...
int openOutput(string outputUrl) {
    int ret = avformat_alloc_output_context2(&outputContext, nullptr, "singlejpeg", outputUrl.c_str());
    if (ret < 0) {
...
int initEncodeContext(AVStream *inputStream, AVCodecContext **encodeContext) {
    ...
    pictureCodec = avcodec_find_encoder(AV_CODEC_ID_MJPEG);
    ...
    (*encodeContext)->pix_fmt = *pictureCodec->pix_fmts;
...
int main() {
    init();
    int ret = openInput("/Users/chengfangming/Downloads/ffmpeg.flv");
    if (ret < 0) {
//        goto Error;
    }
    ret = openOutput("/Users/chengfangming/Downloads/test.jpg");
    if (ret < 0) {
//        goto Error;
    }
